...
function errorMessage(error: unknown): string { if (error instanceof ApiError || error instanceof ApiTimeoutError || error instanceof Error) return error.message; return 'The action could not be completed.'; }
...
export default function HomePage() {
  ...
  async function openBook(book: Book) { setSelectedBook(book); setSelectedChapter(null); setSelectedSegment(null); setSegments([]); setDetailLoading(true); setError(''); try { const [bookResponse, chapterResponse, summaryResponse] = await Promise.all([api.get<Book>(`/api/v1/books/${book.id}`), api.get<Paginated<Chapter>>(`/api/v1/books/${book.id}/chapters?page=1&page_size=50`), api.get<QualitySummary>(`/api/v1/books/${book.id}/quality-summary`)]); setSelectedBook(bookResponse); setChapters(chapterResponse.items || []); setQualitySummary(summaryResponse); } catch (requestError) { setError(errorMessage(requestError)); } finally { setDetailLoading(false); } }
  async function openChapter(chapter: Chapter) { setSelectedChapter(chapter); setSelectedSegment(null); setDetailLoading(true); setError(''); try { const response = await api.get<Paginated<Segment>>(`/api/v1/chapters/${chapter.id}/segments?page=1&page_size=100`); setSegments(response.items || []); } catch (requestError) { setError(errorMessage(requestError)); } finally { setDetailLoading(false); } }
  async function openSegment(segment: Segment) { setSelectedSegment(segment); setError(''); const [jobResponse, reportResponse] = await Promise.allSettled([api.get<TranslationJob[]>(`/api/v1/segments/${segment.id}/translation-jobs?page=1&page_size=20`), api.get<QualityReport>(`/api/v1/segments/${segment.id}/quality-report`)]); if (jobResponse.status === 'fulfilled') setJobs(jobResponse.value); if (reportResponse.status === 'fulfilled') setQualityReport(reportResponse.value); }

  useEffect(() => { if (!selectedJob || selectedJob.status === 'completed' || selectedJob.status === 'failed') return; const controller = new AbortController(); void pollUntilTerminal(() => api.get<TranslationJob>(`/api/v1/translation-jobs/${selectedJob.id}`, { signal: controller.signal }), { signal: controller.signal, intervalMs: 1200, onUpdate: (job) => { setSelectedJob(job); setJobs((current) => current.map((item) => item.id === job.id ? job : item)); } }).catch((pollError) => { if (!(pollError instanceof DOMException && pollError.name === 'AbortError')) setError(errorMessage(pollError)); }); return () => controller.abort(); }, [selectedJob?.id]);

  async function upload(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!uploadFile) { setError('Choose an EPUB, DOCX, or PDF file.'); return; } const validationError = validateUpload(uploadFile); if (validationError) { setError(validationError); return; } const body = new FormData(); body.append('file', uploadFile); setBusy(true); setError(''); setNotice(''); try { await api.upload('/api/v1/books/upload', body); setUploadFile(null); setNotice('Document uploaded and queued for ingestion.'); await loadBooks(); } catch (requestError) { setError(errorMessage(requestError)); } finally { setBusy(false); } }
