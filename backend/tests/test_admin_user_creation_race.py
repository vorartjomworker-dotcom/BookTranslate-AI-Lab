...
2. The real PostgreSQL unique-index path, bypassing the pre-check to prove the
   `IntegrityError` -> 409 mapping (and session recovery via rollback) works
   even when two requests race past the `SELECT` check.
...
def test_sequential_duplicate_email_returns_safe_409(auth_client, async_session_factory) -> None:
    ...
    second = auth_client.post(
        "/api/v1/admin/users",
        json={"email": "  duplicate.user@example.com  ".strip(), "password": "second-password-1", "role": "viewer"},
        headers=headers,
    )
...
# ---------------------------------------------------------------------------
# Layer 2: real PostgreSQL unique-index path, bypassing the pre-check
# ---------------------------------------------------------------------------
...
async def _delete_by_email(session: AsyncSession, *emails: str) -> None:
    for email in emails:
        await session.execute(text("DELETE FROM users WHERE normalized_email = :email"), {"email": email})
    await session.commit()


@pytestmark_pg
@pytest.mark.asyncio
async def test_duplicate_email_db_constraint_returns_conflict_and_session_recovers(monkeypatch) -> None:
    email = "race-db-constraint@example.com"
    followup_email = "race-db-followup@example.com"
    engine, factory = _build_engine_and_factory()
    try:
        async with factory() as setup_session:
            await _delete_by_email(setup_session, email, followup_email)
            existing = User(email=email, password_hash=hash_password("existing-password-1"), role="viewer", is_active=True)
            setup_session.add(existing)
            await setup_session.commit()

        # Simulate the TOCTOU race: the pre-check reports "no existing user" even
        # though a concurrent request has already committed the same normalized
        # email, so only the DB unique index can catch the conflict.
        monkeypatch.setattr(UserRepository, "get_by_normalized_email", AsyncMock(return_value=None))

        async with factory() as session:
            payload = UserCreate(email=email, password="new-password-123", role="viewer")
        ...
        async with factory() as cleanup_session:
            await _delete_by_email(cleanup_session, email, followup_email)
...
def test_helper_only_matches_normalized_email_unique_violation() -> None:
    duplicate_email_orig = SimpleNamespace(sqlstate="23505", constraint_name="ix_users_normalized_email")
    ...
    assert _is_duplicate_user_email_integrity_error(IntegrityError("s", "p", duplicate_email_orig)) is True
