...
def summarize_case_metrics(rows: Iterable[dict[str, object]]) -> dict[str, float | int]:
    ...
    return {
        ...
        "success_rate": round_metric((success_count / case_count) * 100.0),
        "failure_rate": round_metric((failure_count / case_count) * 100.0),
        "average_latency_ms": round_metric(sum(latencies) / case_count),
        ...
        "qa_pass_rate": round_metric((qa_passed / case_count) * 100.0),
...
def summarize_category_metrics(rows: Iterable[dict[str, object]]) -> dict[str, dict[str, float | int]]:
    grouped: defaultdict[str, list[dict[str, object]]] = defaultdict(list)
    for row in rows:
        category = str(row.get("category") or "uncategorized")
        grouped[category].append(row)
    return {category: summarize_case_metrics(grouped[category]) for category in sorted(grouped)}
